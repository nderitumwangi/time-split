...
(function (factory) {
  ...
  function mergeOptions(target) {
    function mergeIntoTarget(name, value) {
      if (
        ...
        // only use values that are set and not undefined
        target[name] = value;
      }
    }
  ...
  fc.datepickerLang = function (langCode, datepickerLangCode, options) {
    ...
    if ($.datepicker) {
      ...
      $.datepicker.regional[datepickerLangCode] = $.datepicker.regional[
        langCode
      ] = options; // alias
  ...
  function Calendar(element, instanceOptions) {
    ...
    function renderView(delta, viewName) {
      ...
      if (!currentView && viewName) {
        ...
        currentView.el = $(
          "<div class='fc-view fc-" + viewName + "-view' />"
        ).appendTo(content);
 ...
 -----------------------------------------------------------------------------*/
    ...
    function zoomTo(newDate, viewName) {
      ...
      if (!viewName || fcViews[viewName] === undefined) {
        ...
        match = viewStr.match(
          new RegExp("\\w+" + capitaliseFirstLetter(viewName))
        );
...
----------------------------------------------------------------------------------------------------------------------*/
 ...
 -----------------------------------------------------------------------------*/
    ...
    function buildEvent(data, source) {
      ...
      out._id =
        data._id ||
        (data.id === undefined ? "_fc" + eventGUID++ : data.id + "");
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  function dateCompare(a, b) {
    // works with Moments and native Dates
    return a - b;
  }
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  FCMoment.prototype.isSame = function (input, units) {
    ...
    if (units) {
      ...
      return (
        moment.fn.isSame.call(this, input) &&
  ...
  DragListener.prototype = {
    ...
    mousemove: function (ev) {
      ...
      if (!this.isDragging) {
        ...
        distanceSq =
          Math.pow(ev.pageX - this.mouseX0, 2) +
          Math.pow(ev.pageY - this.mouseY0, 2);
      ...
      var dragListener = new DragListener(view.coordMap, {
        ...
        cellOver: function (cell, date) {
          ...
          if (view.renderDrag(newStart, newEnd, seg)) {
            // have the view render a visual indication
      ...
      dragListener = new DragListener(this.coordMap, {
        ...
        cellOver: function (cell, date) {
          ...
          if (date.isBefore(start)) {
            // allows comparing ambig to non-ambig
            date = start;
          }
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  $.extend(DayGrid.prototype, {
    ...
    render: function (isRigid) {
      ...
      this.dayEls.each(function (i, node) {
        var date = view.cellToDate(
          Math.floor(i / view.colCnt),
          i % view.colCnt
        );
 ...
 ------------------------------------------------------------------------------------------------------------------*/
    ...
    buildCoords: function (rows, cols) {
      ...
      this.dayEls.slice(0, colCnt).each(function (i, _e) {
        // iterate the first row of day elements
        e = $(_e);
        n = e.offset().left;
        if (i) {
          p[1] = n;
 ...
 ------------------------------------------------------------------------------------------------------------------*/
    ...
    highlightSkeletonHtml: function (startCol, endCol) {
      ...
      if (endCol > startCol) {
        cellHtml +=
          '<td colspan="' + (endCol - startCol) + '" class="fc-highlight" />';
      }
      if (colCnt > endCol) {
  ...
  $.extend(DayGrid.prototype, {
    ...
    renderSegHtml: function (seg, disableResizing) {
      ...
      if (!event.allDay && seg.isStart) {
        timeHtml =
          '<span class="fc-time">' +
          htmlEscape(view.getEventTimeText(event)) +
          "</span>";
      }
      ...
      return (
        ...
        (isRTL
          ...
          : timeHtml + " " + titleHtml) + //
        "</div>" +
        (isResizable ? '<div class="fc-resizer"/>' : "") +
        "</a>"
      );
    },
...
----------------------------------------------------------------------------------------------------------------------*/
 ...
 ------------------------------------------------------------------------------------------------------------------*/
    ...
    getCellDate: function (cell) {
      ...
      return calendar.rezoneDate(
        ...
        view
          .cellToDate(0, cell.col) // View's coord system only accounts for start-of-day for column
 ...
 ------------------------------------------------------------------------------------------------------------------*/
    ...
    renderHighlight: function (start, end) {
      this.highlightEl = $(this.highlightSkeletonHtml(start, end)).appendTo(
        this.el
      );
    },
    ...
    highlightSkeletonHtml: function (start, end) {
      ...
      for (i = 0; i < segs.length; i++) {
        // loop through the segments. one per column
        seg = segs[i];
        ...
        cellHtml +=
          ...
          '<div class="fc-highlight" style="top:' +
          top +
          "px;bottom:-" +
          bottom +
          'px"/>' +
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  $.extend(TimeGrid.prototype, {
    ...
    renderSegHtml: function (seg, disableResizing) {
      ...
      var isDraggable = view.isEventDraggable(event);
      var isResizable =
        !disableResizing && seg.isEnd && view.isEventResizable(event);
      var classes = this.getSegClasses(seg, isDraggable, isResizable);
      var skinCss = this.getEventSkinCss(event);
      var timeText;
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  $.extend(MonthView.prototype, {
    ...
    render: function (date) {
      ...
      this.title = this.calendar.formatDate(
        this.intervalStart,
        this.opt("titleFormat")
      );

      BasicView.prototype.render.call(
        this,
        rowCnt,
        this.getCellsPerWeek(),
        true
...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  function generateAgendaTimeFormat(options, langData) {
    return langData.longDateFormat("LT").replace(/\s*a$/i, ""); // remove trailing AM/PM
  }
 ...
 ------------------------------------------------------------------------------------------------------------------*/
    ...
    getSegs: function () {
      return this.timeGrid
        .getSegs()
        .concat(this.dayGrid ? this.dayGrid.getSegs() : []);
    },
